...
import { CheckCircle, PackageCheck, Plus, XCircle } from "lucide-react"
  ...
  const allRecords = useMemo(() => {
    return [...assignmentRecords]
      .map((r) => {
        const due = new Date(r.dueDate)
        due.setHours(0, 0, 0, 0)
        return { ...r, isOverdue: (r.status === "active" || r.status === "overdue") && due < today }
      })
                            ...
                            <TableCell className="text-right">
                              {r.status === "requested" ? (
                                <div className="flex justify-end gap-2">
                                  <Button size="sm" variant="outline" className="gap-1 text-chart-3" onClick={() => handleApprove(r.id, r.assetName)}>
                                    <CheckCircle className="size-3.5" /> Approve
                                  </Button>
                                  <Button size="sm" variant="outline" className="gap-1 text-destructive" onClick={() => openReject(r.id)}>
                      ...
                      {pending.map((r) => (
                        <TableRow key={r.id}>
                          ...
                          {canManage ? (
                            <TableCell className="text-right">
                              <div className="flex justify-end gap-2">
                                <Button size="sm" variant="outline" className="gap-1.5 text-chart-3" onClick={() => handleApprove(r.id, r.assetName)}>
                                  <CheckCircle className="size-4" /> Approve
                                </Button>
                                <Button size="sm" variant="outline" className="gap-1.5 text-destructive" onClick={() => openReject(r.id)}>
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              {canInitiateReturn && !r.returnDate ? (
                                <Button size="sm" variant="outline" className="gap-1.5" onClick={() => handleInitiateReturn(r.id, r.assetName)}>
                                  <PackageCheck className="size-4" /> Initiate Return
                                </Button>
                              ) : null}
                              {canManage && r.returnDate ? (
                                <Button size="sm" variant="default" className="gap-1.5" onClick={() => handleClose(r.id, r.assetName)}>
                                  Close Return
                                </Button>
                              ) : null}
                            </div>
                    ...
                    <TableBody>
                      {history.map((r) => (
                        <TableRow key={r.id}>
                          ...
                          <TableCell><StatusBadge status={r.status} /></TableCell>
                          <TableCell className="text-sm text-muted-foreground">{r.rejectReason ?? "—"}</TableCell>
                        </TableRow>
                      ))}
                      {history.length === 0 ? (
                        <TableRow>
